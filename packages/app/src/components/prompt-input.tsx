import { useFilteredList } from "@kode/ui/hooks"
...
import { DockShellForm, DockTray } from "@kode/ui/dock-surface"
import { Icon } from "@kode/ui/icon"
import { ProviderIcon } from "@kode/ui/provider-icon"
...
import { ImagePreview } from "@kode/ui/image-preview"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const isPatchingTask = createMemo(() => {
    const text = lastUserText().toLowerCase().trim()
    if (!text) return true // default
    
    const patchingKeywords = [
      "fix", "patch", "implement", "add", "create", "write", "delete", "change", 
      "modify", "update", "build", "refactor", "replace", "rewrite", "run", "make"
    ]
    const analyticalKeywords = [
      "analyze", "explain", "what", "why", "how", "search", "where", "find", 
      "read", "describe", "show", "question", "tell", "greeting", "hello", "hi"
    ]
    
    const hasPatching = patchingKeywords.some(word => text.includes(word))
    const hasAnalytical = analyticalKeywords.some(word => text.includes(word))
    
    if (hasAnalytical && !hasPatching) return false
    return true
  })
  ...
  const activePhaseIndex = createMemo(() => {
    ...
    if (!isPatchingTask()) {
      if (elapsed < 8) return 0 // Plan
      if (elapsed < 20) return 1 // Search
      if (elapsed < 35) return 2 // Analyze
      return 3 // Synthesize
    }
    
    if (diffFiles().length > 0) return 4 // Apply
    if (elapsed < 10) return 0 // Plan
    if (elapsed < 25) return 1 // Critique
    if (elapsed < 45) return 2 // Generate
    return 3 // Verify
  })
  ...
  const progressWidth = createMemo(() => {
    return !isPatchingTask() ? (activePhaseIndex() + 1) * 25 : (activePhaseIndex() + 1) * 20
  })
  ...
  const renderPipeline = () => {
    return (
      <Show
        when={isPatchingTask()}
        fallback={
          <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 py-2 px-1 relative">
            {/* Phase 1: Plan */}
            <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 0 }}>
              <div class="flex items-center gap-2">
                <div class="size-5 rounded-full flex items-center justify-center font-mono text-[10px] font-bold border transition-all"
                  classList={{
                    "bg-[#1c78e3] text-white border-[#1c78e3] shadow-[0_0_12px_rgba(28,120,227,0.4)] animate-pulse": activePhaseIndex() === 0,
                    "bg-[#0f9d58] border-[#0f9d58] text-white": activePhaseIndex() > 0,
                    "bg-v2-background-bg-muted border-border-weak-base text-v2-text-text-faint": activePhaseIndex() < 0
                  }}>
                  <Show when={activePhaseIndex() > 0} fallback="1">
                    <svg class="size-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                  </Show>
                </div>
                <span class="text-[13px] font-semibold text-text-strong" classList={{ "text-[#1c78e3]": activePhaseIndex() === 0 }}>Plan</span>
              </div>
              <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Objectives</span>
            </div>

            <div class="hidden md:block h-[1px] w-8 bg-border-weaker-base shrink-0" />

            {/* Phase 2: Search */}
            <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 1 }}>
              <div class="flex items-center gap-2">
                <div class="size-5 rounded-full flex items-center justify-center font-mono text-[10px] font-bold border transition-all"
                  classList={{
                    "bg-[#1c78e3] text-white border-[#1c78e3] shadow-[0_0_12px_rgba(28,120,227,0.4)] animate-pulse": activePhaseIndex() === 1,
                    "bg-[#0f9d58] border-[#0f9d58] text-white": activePhaseIndex() > 1,
                    "bg-v2-background-bg-muted border-border-weak-base text-v2-text-text-faint": activePhaseIndex() < 1
                  }}>
                  <Show when={activePhaseIndex() > 1} fallback="2">
                    <svg class="size-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                  </Show>
                </div>
                <span class="text-[13px] font-semibold text-text-strong" classList={{ "text-[#1c78e3]": activePhaseIndex() === 1 }}>Search</span>
              </div>
              <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Code Index</span>
            </div>

            <div class="hidden md:block h-[1px] w-8 bg-border-weaker-base shrink-0" />

            {/* Phase 3: Analyze */}
            <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 2 }}>
              <div class="flex items-center gap-2">
                <div class="size-5 rounded-full flex items-center justify-center font-mono text-[10px] font-bold border transition-all"
                  classList={{
                    "bg-[#1c78e3] text-white border-[#1c78e3] shadow-[0_0_12px_rgba(28,120,227,0.4)] animate-pulse": activePhaseIndex() === 2,
                    "bg-[#0f9d58] border-[#0f9d58] text-white": activePhaseIndex() > 2,
                    "bg-v2-background-bg-muted border-border-weak-base text-v2-text-text-faint": activePhaseIndex() < 2
                  }}>
                  <Show when={activePhaseIndex() > 2} fallback="3">
                    <svg class="size-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                  </Show>
                </div>
                <span class="text-[13px] font-semibold text-text-strong" classList={{ "text-[#1c78e3]": activePhaseIndex() === 2 }}>Analyze</span>
              </div>
              <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Semantic Scan</span>
            </div>

            <div class="hidden md:block h-[1px] w-8 bg-border-weaker-base shrink-0" />

            {/* Phase 4: Synthesize */}
            <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 3 }}>
              <div class="flex items-center gap-2">
                <div class="size-5 rounded-full flex items-center justify-center font-mono text-[10px] font-bold border transition-all"
                  classList={{
                    "bg-[#1c78e3] text-white border-[#1c78e3] shadow-[0_0_12px_rgba(28,120,227,0.4)] animate-pulse": activePhaseIndex() === 3,
                    "bg-[#0f9d58] border-[#0f9d58] text-white": activePhaseIndex() > 3,
                    "bg-v2-background-bg-muted border-border-weak-base text-v2-text-text-faint": activePhaseIndex() < 3
                  }}>
                  <Show when={activePhaseIndex() > 3} fallback="4">
                    <svg class="size-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                  </Show>
                </div>
                <span class="text-[13px] font-semibold text-text-strong" classList={{ "text-[#1c78e3]": activePhaseIndex() === 3 }}>Synthesize</span>
              </div>
              <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Response Form</span>
            </div>
          </div>
        }
      >
        <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 py-2 px-1 relative">
          ...
          <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 0 }}>
            ...
            <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Context Graph</span>
          </div>
          ...
          <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 1 }}>
            <div class="flex items-center gap-2">
              ...
              <span class="text-[13px] font-semibold text-text-strong" classList={{ "text-[#1c78e3]": activePhaseIndex() === 1 }}>Critique</span>
            </div>
            <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Self-Correction</span>
          </div>
          ...
          <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 2 }}>
            <div class="flex items-center gap-2">
              ...
              <span class="text-[13px] font-semibold text-text-strong" classList={{ "text-[#1c78e3]": activePhaseIndex() === 2 }}>Generate</span>
            </div>
            <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Synthesis</span>
          </div>
          ...
          <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 3 }}>
            <div class="flex items-center gap-2">
              ...
              <span class="text-[13px] font-semibold text-text-strong" classList={{ "text-[#1c78e3]": activePhaseIndex() === 3 }}>Verify</span>
            </div>
            <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Tiered Gate Check</span>
          </div>

          <div class="hidden md:block h-[1px] w-8 bg-border-weaker-base shrink-0" />

          {/* Apply */}
          <div class="flex-1 flex flex-col gap-1 min-w-0 z-10" classList={{ "opacity-50": activePhaseIndex() < 4 }}>
            <div class="flex items-center gap-2">
              <div class="size-5 rounded-full flex items-center justify-center font-mono text-[10px] font-bold border transition-all"
                classList={{
                  "bg-[#1c78e3] text-white border-[#1c78e3] shadow-[0_0_12px_rgba(28,120,227,0.4)] animate-pulse": activePhaseIndex() === 4,
                  "bg-[#0f9d58] border-[#0f9d58] text-white": activePhaseIndex() > 4,
                  "bg-v2-background-bg-muted border-border-weak-base text-v2-text-text-faint": activePhaseIndex() < 4
                }}>
                <Show when={activePhaseIndex() > 4} fallback="5">
                  <svg class="size-3 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
                </Show>
              </div>
              <span class="text-[13px] font-semibold text-text-strong" classList={{ "text-[#1c78e3]": activePhaseIndex() === 4 }}>Apply</span>
            </div>
            <span class="text-[11px] text-v2-text-text-faint truncate pl-7">Disk Write</span>
          </div>
        </div>
      </Show>
    )
  ...
  const USE_V2_INPUT = import.meta.env.VITE_KODE_CHANNEL !== "prod"
      ...
      <Switch>
        <Match when={USE_V2_INPUT}>
          ...
          >
            ...
            >
              {/* Mission Stage Takeover UI */}
              <div class="w-full bg-[#fafafa] dark:bg-v2-background-bg-deep rounded-[12px] border border-border-weak-base shadow-sm p-4 flex flex-col gap-4 animate-fade-in transition-all">
                {/* Header Row */}
                <div class="flex items-center justify-between border-b border-border-weaker-base pb-3">
                  <div class="flex items-center gap-2">
                    <span class="relative flex h-2 w-2">
                      <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
                      <span class="relative inline-flex rounded-full h-2 w-2 bg-[#1c78e3]"></span>
                    </span>
                    <span class="text-[12px] font-semibold tracking-wider text-v2-text-text-base uppercase">
                      Mission Orchestrator
                    </span>
                  </div>
                  <div class="flex items-center gap-3">
                    <span class="text-[11px] text-v2-text-text-faint font-mono">
                      Fleet Active: {local.agent.current()?.name || "primary"}
                    </span>
                    <button
                      type="button"
                      onClick={abort}
                      class="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-50 hover:bg-red-100 dark:bg-red-950/30 dark:hover:bg-red-900/40 text-red-600 dark:text-red-400 font-medium text-[11px] border border-red-200/50 transition-all cursor-pointer"
                    >
                      <div class="size-2 bg-red-600 dark:bg-red-400 rounded-sm" />
                ...
                {/* Progress bar and active subtask detail */}
                <div class="bg-white dark:bg-v2-background-bg-hover rounded-lg p-3 flex flex-col gap-2 border border-border-weaker-base">
                  <div class="flex items-center justify-between text-[11.5px] font-medium text-v2-text-text-muted">
                    <span class="flex items-center gap-2 min-w-0">
                      <svg class="animate-spin h-3.5 w-3.5 text-[#1c78e3] shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                      </svg>
                      <span class="truncate">
                        <Switch>
                          <Match when={!isPatchingTask()}>
                            <Switch>
                              <Match when={activePhaseIndex() === 0}>Formulating analytical plan & defining queries...</Match>
                              <Match when={activePhaseIndex() === 1}>Searching workspace files & scanning directories...</Match>
                              <Match when={activePhaseIndex() === 2}>Analyzing semantic code patterns & context...</Match>
                              <Match when={activePhaseIndex() === 3}>Synthesizing clean comprehensive explanation...</Match>
                            </Switch>
                          </Match>
                          <Match when={isPatchingTask()}>
                            <Switch>
                              <Match when={activePhaseIndex() === 0}>Building surgical context graph & analyzing files...</Match>
                              <Match when={activePhaseIndex() === 1}>Agent fleet critiquing and refining solution paths...</Match>
                              <Match when={activePhaseIndex() === 2}>Generating solution patches via LLM synthesis...</Match>
                              <Match when={activePhaseIndex() === 3}>Running Tiered Gate Pipeline (AST + Typecheck + Async Test)...</Match>
                              <Match when={activePhaseIndex() === 4}>Applying refined file changes to workspace disk...</Match>
                            </Switch>
                          </Match>
                        </Switch>
                      </span>
                    </span>
                    <span class="font-mono text-[11px] text-v2-text-text-faint shrink-0">
                      Elapsed: {elapsedTime()}s
                    </span>
                  </div>
                  <div class="w-full bg-border-weaker-base h-1.5 rounded-full overflow-hidden relative">
                    <div class="absolute inset-y-0 left-0 bg-[#1c78e3] rounded-full transition-all duration-500" style={{ width: `${progressWidth()}%` }}></div>
                  </div>
                </div>
              </div>
            </Show>
