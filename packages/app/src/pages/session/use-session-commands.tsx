...
import { previewSelectedLines } from "@kode/ui/pierre/selection-bridge"
...
import { findLast } from "@kode/core/util/array"
import { createSessionTabs } from "@/pages/session/helpers"
import { extractPromptFromParts } from "@/utils/prompt"
import { UserMessage } from "@kode/sdk/v2"
...
export const useSessionCommands = (actions: SessionCommandContext) => {
  ...
  const sessionCmds = () => [
    ...
    // KODE CLI SLASH COMMANDS
    sessionCommand({
      id: "kode.plan",
      title: "Plan Task (Kode)",
      description: "Instruct Kode to build a surgical 8K context graph before executing",
      slash: "plan",
      onSelect: () => {
        prompt.set([{ type: "text", content: "/plan ", start: 0, end: 6 }])
        focusInput()
      },
    }),
    sessionCommand({
      id: "kode.verify",
      title: "Verify Code (Kode)",
      description: "Run the Tiered Gate Pipeline verification check on the current code",
      slash: "verify",
      onSelect: () => {
        prompt.set([{ type: "text", content: "/verify ", start: 0, end: 8 }])
        focusInput()
      },
    }),
    sessionCommand({
      id: "kode.loop",
      title: "Full Loop (Kode)",
      description: "Execute the full Plan -> Generate -> Verify -> Apply -> Test cycle",
      slash: "loop",
      onSelect: () => {
        prompt.set([{ type: "text", content: "/loop ", start: 0, end: 6 }])
        focusInput()
      },
    }),
  ]
