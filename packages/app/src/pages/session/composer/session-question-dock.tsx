...
export const SessionQuestionDock: Component<{ request: QuestionRequest; onSubmit: () => void }> = (props) => {
  ...
  return (
    <div class="fixed inset-0 z-[9999] flex items-center justify-center bg-background-base/80 backdrop-blur-sm" onKeyDown={nav}>
      <div class="w-full max-w-2xl bg-surface-raised-base rounded-xl shadow-[var(--shadow-xl-border-base)] flex flex-col max-h-[85vh] overflow-hidden border border-border-interactive-base" ref={(el) => (root = el)}>
        <div class="flex items-center justify-between px-6 py-4 border-b border-border-weak-base bg-surface-raised-stronger">
          <div class="text-16-medium text-text-strong tracking-wide">{summary()}</div>
          <div class="flex items-center gap-1">
            <For each={questions()}>
              {(_, i) => (
                <div
                  class="h-2 w-8 rounded-full transition-colors cursor-pointer"
                  classList={{
                    "bg-border-interactive-base": i() === store.tab,
                    "bg-border-strong-base": i() !== store.tab && answered(i()),
                    "bg-border-weak-base": i() !== store.tab && !answered(i())
                  }}
                  onClick={() => jump(i())}
                />
              )}
      ...
      <div class="flex items-center justify-between px-6 py-4 border-t border-border-weak-base bg-surface-raised-stronger">
