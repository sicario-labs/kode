...
import "./styles.css"
import { useTheme } from "@kode/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.KODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "kode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__KODE__ ??= {}
  const pending = window.__KODE__.deepLinks ?? []
  window.__KODE__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("kode.global.dat").getItem("language")
