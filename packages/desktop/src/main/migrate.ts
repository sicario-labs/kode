...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "xyz.trykode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `kode.settings.dat` is special: it maps to the `kode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"kode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // kode.settings.dat → the electron settings store ("kode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "kode.global.dat").
  const storeName = filename === "kode.settings.dat" ? "kode.settings" : filename
